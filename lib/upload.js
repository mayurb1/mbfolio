...
const UPLOAD_KINDS = {
  ...
  logo: {
    ...
    prefix: 'logo',
    validate: isValidLogoType,
    invalidTypeMessage: 'Invalid file type. Only SVG files are allowed.',
    sizeLimit: FILE_SIZE_LIMITS.LOGO,
    sizeLimitMessage: 'File size too large. Maximum size is 1MB.',
    // No sharp optimization: keep the SVG as a vector instead of rasterizing it.
    optimize: null,
    resourceType: 'image',
    allowedFormats: ['svg'],
  },
...
export async function uploadLogo(file, userId) {
  return uploadFile(file, 'logo', userId)
}
